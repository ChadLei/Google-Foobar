...
def solution(l, t):
	# Map all sums at indexes they were calculated at.
	sums = {0:-1}
	# Keep track of the total sum we've seen so far.
	totalSum = 0
	# Visit each number to check whether we've found the target.
	for i in range(len(l)):
		totalSum += l[i]
		if totalSum not in sums:
			sums[totalSum] = i
		# If the difference between the sum of all visited numbers minus target has already
		# been found, then we've found our answer.
		if totalSum - t in sums:
			return [sums[totalSum - t] + 1, i]
	return [-1, -1]



l1 = [[4, 3, 5, 7, 8], 12, [0,2]]
l2 = [[4, 3, 10, 2, 8], 12, [2,3]]
l3 = [[4, 12, 10, 2, 8], 12, [1,1]]
l4 = [[1, 2, 3, 4], 15, [-1,-1]]
l5 = [[1, 2, 3], 6, [0,2]]
l6 = [[4, 3, 8, 2, 10], 12]
l7 = [[4, 3, 2, 1, 10, 2], 12]
...
for i in tests:
	if (solution(i[0],i[1]) != i[2]):
		print(f"Number {counter}: ", end="")
		print(i[0])
	counter += 1

# print(solution(l7[0],l7[1]))
